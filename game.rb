...
class Game
  ...
  def check_upper_left_diagonal(start_x, start_y)
    ...
    def check_upper_right_diagonal(start_x, start_y)
    x = start_x
    y = start_y
    counter = 0
    current_player = nil
  ...
  def diagonal_winner?
    ...
    @size.times do |start_y|
      return true if check_upper_left_diagonal(0, start_y) || check_upper_right_diagonal(0, start_y)
    end

    # Look diagonally (upper-left to lower-right) along the left column
    # Return true if connect four found
    @size.times do |start_x|
      return true if check_upper_left_diagonal(start_x, 0) || check_upper_right_diagonal(start_x,0)
    end
  ...
  def shift_down(column)
	...
	above_saved_spot = BLANK_SPACE
    saved_spot = BLANK_SPACE
	lowest_blank = find_lowest_blank(column) +1
	#puts lowest_blank

	lowest_blank.times do |x|
	 saved_spot = @board[x][column]
     @board[x][column] = above_saved_spot
     above_saved_spot = saved_spot
	end
  ...
  def find_lowest_blank(column)
     lowest = -1
	 top_found = false
	@size.times do |x|
		if (@board[x][column] != BLANK_SPACE) && !top_found#Detects when to start caring about blanks
			top_found = true

		end
		if (@board[x][column] == BLANK_SPACE) && top_found 
			lowest = x
  ...
  def pieces_fall
    ...
    @size.times do |column|
      if find_lowest_blank(column) == -1 || all_blanks_in_column?(column)
         # Skip this column, it is either full, shifted properly, or entirely empty
      else
        # Shift column down until the lowest checker is in the bottom spot
        shift_down column until find_lowest_blank(column) == -1
      end
    end
  end
  ...
  def show_results
    display_board
    if all_spots_filled?
	  if winner? #Fixed automatic tie on last possible play, even if it won the game
		if @turn == O_PLAYER
        puts 'Player X won!'
      else
        puts 'Player O won!'
  ...
  def display_2d_array(arr)
    arr[0].size.times do |x|
      print x%10 # Fixed requirement 9 by modding index by 10
    end
    puts
